...
class Camera(name: String, pose: Transform3d) {
    ...
    private val singleTagStds: Matrix<N3, N1> = VecBuilder.fill(3.0, 3.0, 4.0)
    private val multiTagStds: Matrix<N3, N1> = VecBuilder.fill(0.3, 0.3, 0.75)
    ...
    private fun updateStds(pose: EstimatedRobotPose?, targets: List<PhotonTrackedTarget>) {
        ...
        // TW: Also, the only case that the getTagPose would be null is if the frc.robot detects an
